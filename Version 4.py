...
class MultiCameraTracker:
    ...
    def select_person_at_point(self, camera_id: int, point_x: int, point_y: int) -> Optional[int]:
        try:
            with self.lock:
                ...
                # Find matching box using vectorized operations
                x1, y1, x2, y2 = boxes.T
    ...
    def update_selected_person_tracking(self, features: torch.Tensor, boxes: np.ndarray, camera_id: int) -> None:
        """Update tracking with improved position matching and motion prediction"""
        try:
            with self.lock:
                ...
                # Get cached or compute target features
                cache_key = (self.selected_person_id, camera_id)
                with self.cache_lock:
                    if cache_key in self.feature_cache:
                        target_features = self.feature_cache[cache_key]
                    else:
                        target_features = selected_track.get_avg_features()
                        self.feature_cache[cache_key] = target_features

                # Compute appearance similarity
                similarities = self.max_cosine_sim(features, target_features.unsqueeze(0))
                
                # Add motion prediction for better tracking
                if len(selected_track.location_history) > 0:
                    ...
                    predicted_center = last_center + selected_track.velocity
                    ...
                    distances = np.linalg.norm(current_centers - predicted_center, axis=1)
                    distance_weights = 1 / (1 + distances)
                    
                    # Combine appearance and motion scores
                    combined_scores = similarities.cpu().numpy() * distance_weights
    ...
    def process_frame(self, frame: np.ndarray, camera_id: int) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Person detection with timeout
            detection_timeout = 1.0  # 1 second timeout
            detection_start = time.time()
            boxes = self.feature_extractor.detect_persons(rgb_frame)
            
            if time.time() - detection_start > detection_timeout:
                logger.warning("Person detection timeout")
                return frame
            ...
            if time.time() - feature_start > feature_timeout:
            ...
            # Update detection results atomically
            with self.lock:
            ...
            tracking_thread = threading.Thread(
                target=self.update_selected_person_tracking,
                args=(features, boxes, camera_id),
                daemon=True
            )
            tracking_thread.start()
            tracking_thread.join(timeout=0.1)  # Short timeout to prevent hanging
    ...
    def draw_tracking(self, frame: np.ndarray, boxes: np.ndarray, camera_id: int) -> np.ndarray:
        """Enhanced visualization of tracking with motion trail"""
        try:
            vis_frame = self.frame_pool.get(frame.shape)
            np.copyto(vis_frame, frame)
            ...
            if self.selected_person_id in self.tracks:
                ...
                # Only draw if this camera was the last to see the person
                if track.last_camera_id == camera_id:
                    if len(track.location_history) > 0:
                        ...
                        # Draw bounding box
                        cv2.rectangle(vis_frame, (x1, y1), (x2, y2), track.color, 2)
                        
                        # Add ID and confidence
                        label = f"ID: {track.id}"
                        cv2.putText(vis_frame, label, (x1, y1 - 10), 
                                  cv2.FONT_HERSHEY_SIMPLEX, 0.7, track.color, 2)
            ...
            # Draw status overlay
            status = f"Tracking ID: {self.selected_person_id}" if self.selected_person_id else "No person selected"
...
def main():
    ...
    try:
        ...
        sources = {
            ...
            1: r"E:\2025\Hack-2-detect\TestModelVideo.mp4",
            # 2: "path/to/video/file.mp4"
        }
